...
func main() {
	...
	const file = "onehot_weights.json"
	var model *MLP

	if _, err := os.Stat(file); err == nil {
		fmt.Println("Loading One-Hot Model...")
		data, _ := os.ReadFile(file)
		json.Unmarshal(data, &model)
	} else {
		fmt.Println("Training One-Hot Model (Cross-Entropy)...")
		model = NewMLP(InputDim, 64, InputDim)
		lr := 0.05
		for e := 0; e < 100000; e++ {
			s := generateRandomString(StrLen)
			loss := model.TrainCE(encode(s), encode(reverse(s)), lr)
			if e%10000 == 0 {
				fmt.Printf("Epoch %d/100000 | Avg Loss: %.4f\n", e, loss/StrLen)
			}
		}
