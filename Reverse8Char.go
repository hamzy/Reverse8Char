...
func main() {
	...
	const file = "concurrent_onehot_weights.json"
	var model *MLP

	// 1. Load or Initialize Model
	if _, err := os.Stat(file); err == nil {
		fmt.Println("Loading weights from file...")
		data, _ := os.ReadFile(file)
		json.Unmarshal(data, &model)
	} else {
		fmt.Println("No weights found. Initializing deep network...")
		model = NewMLP(InputDim, 128, InputDim) // Increased hidden layer to 128 for better accuracy

		// 2. Setup Parallel Training
		const numWorkers = 8
		const totalSamples = 150000
		jobs := make(chan int, totalSamples)
		var wg sync.WaitGroup

		fmt.Printf("Starting parallel training with %d workers...\n", numWorkers)
		start := time.Now()
		...
		for w := 0; w < numWorkers; w++ {
			...
			go func() {
				...
				for range jobs {
					...
					// TrainCE now includes m.mu.Lock() inside its definition
					model.TrainCE(encode(s), encode(reverse(s)), 0.05)
				}
			}()
		}
		...
		for i := 0; i < totalSamples; i++ {
			jobs <- i
			if i%30000 == 0 {
				fmt.Printf("Progress: %d samples queued...\n", i)
			}
		}
		close(jobs)
		wg.Wait()

		fmt.Printf("Training complete in %v. Saving weights...\n", time.Since(start))
		d, _ := json.Marshal(model)
		os.WriteFile(file, d, 0644)
	}

	// 3. Validation and Testing
	testStrings := []string{"onionsys", "goisfast", "neuralgo"}

	fmt.Println("\n--- FINAL VALIDATION ---")
	for _, test := range testStrings {
		// Use model.mu.Lock if PredictRaw doesn't have it,
		// though single-thread prediction is usually safe.
